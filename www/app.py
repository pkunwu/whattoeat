...
def init_jinja2(app, **kw):
    ...
    if path is None:
        path = os.path.join(os.path.dirname(os.path.abspath('templates')),'templates')
...
def datetime_filter(t):
    ...
    if delta<60:
        return '1 min ago'
    if delta<3600:
        return '%s mins ago' %(3600//60)
    if delta<86400:
        return '%s hours ago' %(delta//3600)
    if delta<604800:
        return '%s days ago' %(delta//86400)
    dt = datetime.fromtimestamp(t)
    return '%s years %s months %s days' %(dt.year, dt.month, dt.day)

async def init(loop):
    await db_orm.create_pool(loop, host = '127.0.0.1', port = 3306, user = 'www-data', password = 'www-data', db = 'awesome')
    app = web.Application(
        loop = loop, 
